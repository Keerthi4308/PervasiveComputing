    ...
    private final BACtrackAPICallbacks mCallbacks = new BACtrackAPICallbacks() {
        ...
        public void BACtrackError(int errorCode) {
            ...
            if(errorCode == 4)
            {
                ...
                myRef.child(user.getUid().toString()).push().setValue(new LogSnip("User Needs to Charge the device, low battery reported."));
                Toast.makeText(InitiateActivity.this, "The Breathylizer has low battery. Please Charge your device befire usage", Toast.LENGTH_LONG);

            }
            else
            {
                myRef.child(user.getUid().toString()).push().setValue(new LogSnip("Error in Device reported: " + errorCode));
            }
            disconnect();
            Intent i = new Intent(getApplicationContext(), RestartActivity.class);
